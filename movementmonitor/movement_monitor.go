...
// Model is the model triple for the movement-monitor sensor component.
var Model = resource.NewModel("katie-viam", "oreo-watcher", "movement-monitor")

func init() {
	resource.RegisterComponent(sensor.API, Model, resource.Registration[sensor.Sensor, *Config]{
		Constructor: newMovementMonitor,
	})
}
...
func newMovementMonitor(ctx context.Context, deps resource.Dependencies, conf resource.Config, logger logging.Logger) (sensor.Sensor, error) {
...
// Readings returns the next completed movement session, or ErrNoCaptureToStore if none is ready.
func (m *movementMonitor) Readings(ctx context.Context, extra map[string]interface{}) (map[string]interface{}, error) {
	s := m.activity.PopMovementSession()
	if s == nil {
...
func (m *movementMonitor) DoCommand(ctx context.Context, cmd map[string]interface{}) (map[string]interface{}, error) {
	return nil, fmt.Errorf("no commands supported")
}

func (m *movementMonitor) Close(ctx context.Context) error {
	return nil
}
